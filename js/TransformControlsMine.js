...
THREE.TransformControls = function ( camera, domElement ) {
	...
	this.pointerMove = function( pointer ) {
		...
		if ( mode === 'translate' ) {
			...
			var ROTATION_SPEED = 22 / worldPosition.distanceTo( _tempVector.setFromMatrixPosition( this.camera.matrixWorld ) );
...
THREE.TransformControlsGizmo = function () {
	...
	matRed.color.set( 0xDD124E );

	var matGreen = gizmoMaterial.clone();
	matGreen.color.set( 0x00FFFF );

	var matBlue = gizmoMaterial.clone();
	matBlue.color.set( 0x2E67EA );
	...
	matLineRed.color.set( 0xDD124E );

	var matLineGreen = gizmoLineMaterial.clone();
	matLineGreen.color.set( 0x00FFFF );

	var matLineBlue = gizmoLineMaterial.clone();
	matLineBlue.color.set( 0x2E67EA );
	...
	this.updateMatrixWorld = function () {
		...
		for ( var i = 0; i < handles.length; i++ ) {
			...
			handle.scale.set( 1, 1, 1 ).multiplyScalar( eyeDistance * this.size /  6 );    //size of control
